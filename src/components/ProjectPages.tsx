      ...
      <div className="mb-5" key={project.id}>
        ...
        <div className="text-sm mt-1">
          {project.links.map((link: { label: string; url: string }) => (
            <a href={link.url} className="text-indigo-500 mr-3" key={link.url}>{link.label}</a>
          ))}
        </div>
      </div>
    ))}
...
export const ProjectDetail = () => {
  ...
  return (
    <section className="max-w-3xl mx-auto px-4 py-8">
      <Link to="/projects" className="text-gray-500 hover:underline block mb-6">&larr; Projects</Link>
